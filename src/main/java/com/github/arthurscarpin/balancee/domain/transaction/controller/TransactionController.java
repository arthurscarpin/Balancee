...
public class TransactionController {
    ...
    public ResponseEntity<?> createPostTransaction(@RequestBody TransactionRequest transactionRequest) {
        try {
            TransactionResponse transactionCreated = service.create(transactionRequest);
    ...
    public ResponseEntity<?> findByIdGetTransaction(@PathVariable Long id) {
        try {
            TransactionResponse transaction = service.findById(id);
    ...
    public ResponseEntity<?> findByUserIdGetTransaction(@PathVariable Long userId) {
        try {
            List<TransactionResponse> transaction = service.findByUserId(userId);
            return ResponseEntity.ok(transaction);
        } catch (BusinessException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(e.getMessage());
    ...
    public ResponseEntity<?> findByUserIdAndTypeGetTransaction(
        ...
        try {
            List<TransactionResponse> transaction = service.findByUserIdAndType(userId, type);
            return ResponseEntity.ok(transaction);
        } catch (BusinessException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(e.getMessage());
    ...
    public ResponseEntity<?> findByUserIdAndYearAndMonthGetTransaction(
        ...
        try {
            List<TransactionResponse> transaction = service.findByUserIdAndYearAndMonth(userId, type, year, month);
            return ResponseEntity.ok(transaction);
        } catch (BusinessException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(e.getMessage());
    ...
    @Operation(
            summary = "Report monthly Balance.",
            description = "Calculate the total monthly balance.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Value of monthly balance."),
    })
    ...
    public ResponseEntity<?> updateByIdPutTransaction(
            @PathVariable Long id,
            @RequestBody TransactionRequest transactionRequest
    ) {
        try {
            TransactionResponse transactionUpdated = service.updatePutById(id, transactionRequest);
    ...
    @Operation(
            ...
            description = "Partially updates the details of an existing transaction by its ID."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Transaction partially updated successfully."),
            @ApiResponse(responseCode = "404", description = "Transaction not found.")
    })
    public ResponseEntity<?> partialUpdateByIdPatchTransaction(
            @PathVariable Long id,
            @RequestBody TransactionRequest transactionRequest
    ) {
        try {
            TransactionResponse transactionUpdated = service.updatePatchById(id, transactionRequest);
            return ResponseEntity.ok(transactionUpdated);
        } catch (BusinessException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(e.getMessage());
