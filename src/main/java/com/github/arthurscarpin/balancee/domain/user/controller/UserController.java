...
public class UserController {
    ...
    public ResponseEntity<?> createPostUser(@RequestBody UserRequest userRequest) {
        try {
            UserResponse userCreated = service.create(userRequest);
    ...
    public ResponseEntity<?> updateByIdPutUser(@PathVariable Long id, @RequestBody UserRequest userRequest) {
        try {
            UserResponse userUpdated = service.updatePutById(id, userRequest);
    ...
    @Operation(
            ...
            description = "Partially updates the details of an existing user by its ID."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User partially updated successfully."),
            @ApiResponse(responseCode = "404", description = "User not found.")
    })
    public ResponseEntity<?> updateByIdPatchUser(@PathVariable Long id, @RequestBody UserRequest userRequest) {
        try {
            UserResponse userUpdated = service.updatePatchById(id, userRequest);
            return ResponseEntity.ok(userUpdated);
        } catch (BusinessException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(e.getMessage());
